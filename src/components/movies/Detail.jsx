...
import { URLDetail } from "../../api/urlMovies";
...
const DetailMovie = () => {
  ...
  const [detailMovie, setDetailMovie] = useState(DataDetailMovie);
  useEffect(() => {
  ...
  const getDetail = async () => {
    if (id !== "12345") {
      try {
        const response = await fetch(URLDetail).then((res) => res.json());
        const result = await response.items;
        if (!result) return navigate("/", { replace: true });
